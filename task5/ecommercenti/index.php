...
?>
		...
		<?php
						  ...
						  public function newproduct(){
							$res=  "SELECT `id`, `name_en` ,`image`, `status`, `price` FROM `product` 
							    WHERE `status` =1
								  order by `created_at` DESC ,`name_en`DESC  LIMIT 4";
								  return $this->con->query($res);
								 
						}
						public function mostrate(){
							$res="SELECT `review_id` , `product_id` , `product`.`id`,`name_en`,`image`,`product`.`status`,`price`,`reviews`.`id`, COUNT(`product_review`.`product_id`) AS `review-product`, AVG(`reviews`.`rate`) AS `rev_rat` FROM `product` JOIN `product_review` ON `product_review`.`product_id` = `product`.`id` JOIN `reviews` ON `product_review`.`review_id` = `reviews`.`id`WHERE `product`.`status`=1 GROUP BY `product_review`.`product_id` order BY COUNT(`product_id`) DESC, AVG(`reviews`.`rate`) DESC LIMIT 4";
							return $this->con->query($res);

						}
						public function mostorder(){
							$res="SELECT `order_id` , `product_id` , `product`.`id`,`name_en`,`image`,`product`.`status`,`price`,`order`.`id`, COUNT(`product_order`.`product_id`) AS `order-product` FROM `product` JOIN `product_order` ON `product_order`.`product_id` = `product`.`id` JOIN `order` ON `product_order`.`order_id` = `order`.`id`WHERE `product`.`status`=1 GROUP BY `product_order`.`product_id` order BY COUNT(`product_id`) DESC LIMIT 4";
							return $this->con->query($res);
                ...
                <div class="tab-content jump">
                    <div class="tab-pane active">
					  ...
					  <?php
							...
							if($connect->newproduct()->num_rows > 0){
								$results = $connect->newproduct()->fetch_all(MYSQLI_ASSOC);
                            ...
                            <div class="product-wrapper-single">
								<div class="product-wrapper mb-30">
									<div class="product-img">
										<a href="product-details.html"">
										<img style="width: 270px; height:250px" alt="" src="assets/img/new-product/<?=$resl['image']?>">
										</a>
									...
									<div class="product-content text-left">
										<div class="product-hover-style">
											<div class="product-title">
												<h4>
													<a href="product-details.html"><?=$resl['name_en']?></a>
												</h4>
											</div>
											<div class="cart-hover">
												<h4><a href="product-details.html">+ Add to cart</a></h4>
            ...
            <div class="container">
                ...
                <div class="tab-content jump">
                    <div class="tab-pane active">
					  ...
					  <?php
                        	$connect = new connection ;
							if($connect->mostrate()->num_rows > 0){
								$results = $connect->mostrate()->fetch_all(MYSQLI_ASSOC);
							foreach($results as $resl){?>
            ...
            <div class="container">
                ...
                <div class="tab-content jump">
                    <div class="tab-pane active">
					  ...
					  <?php
                        	$connect = new connection ;
							if($connect->mostorder()->num_rows > 0){
								$results = $connect->mostorder()->fetch_all(MYSQLI_ASSOC);
							foreach($results as $resl){?>
